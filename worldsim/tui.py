...
class WorldSimApp(App[None]):
    ...
    def __init__(self, store: CampaignStore, engine: WorldEngine | None = None) -> None:
        super().__init__()
        self.store = store
        self.engine = engine or WorldEngine()
        self.director = MockDirector(self.engine.seed)
    ...
    def _compose_game_screen(self) -> ComposeResult:
        with Container(id="game-screen"):
            ...
            yield Static(
                "Press 'm' or Esc to focus map. Arrows move. Shift+Arrows pan. 'c' centers. 'f' toggles follow.",
                id="footer-note",
            )
    ...
    def _start_new_campaign(self) -> None:
        ...
        world = self.engine.create_world()
        ...
        self.store.save(world, player, memory)
        self.follow_player = True
        self._center_camera_on_player()
    ...
    def _area_choices(self, location: Location | None) -> list[str]:
        ...
        return self._wilderness_areas()

    def _wilderness_areas(self) -> list[str]:
        if self.session is None:
            return ["Camp Ring", "Survey Point", "Trail Break"]
    ...
    def _display_area(self, area: str | None) -> str:
        return area if area is not None else "None"

    def _area_scene_text(self, location: Location | None) -> str:
        assert self.session is not None
        area_name = self.session.entered_area or "Unknown Area"
        zone = self._area_zone_name()
        threat = self.session.entered_area_hazard or "no immediate hazard"
        npc = self.session.entered_area_npc or "no clear local figure"
        if location is not None:
            return (
                f"{area_name} is a tighter slice of {location.name}. "
                f"You are at the {zone}. The scene leans toward {self.session.entered_area_theme or 'uncertain frontier detail'}, "
                f"with {threat} in play and {npc} nearby."
            )
        return (
            f"{area_name} is a local patch of frontier terrain. "
            f"You are at the {zone}, with {threat} in play and {npc} nearby."
        )
    ...
    def _area_zone_name(self) -> str:
        assert self.session is not None
        return ["approach", "interior", "deep point"][self.session.entered_area_step]
    ...
    def _area_hazard(self) -> str:
        ...
        return hazards[biome][self._area_hash() % 3]
    ...
    def _area_npc(self) -> str | None:
        ...
        return choices[self._area_hash() % len(choices)]

    def _area_hash(self) -> int:
        assert self.session is not None
        token = self.session.entered_area or self.session.selected_area or "frontier"
        return sum(ord(char) for char in token) + self.session.world.seed
